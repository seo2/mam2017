                        ...
                        <div class="container">
                        	<div class="row" id="caja_tienda">
								...
								if($_GET['busqueda']){
									...
									if($_GET['pagina']){
										$desde 	 = 12 * ($_GET['pagina'] - 1);
										$tiendas = $db->rawQuery("select * from pak_tiendas where idioma = $idioma and tipo NOT IN(196,199,193) order by nombre limit $desde, 12");
									}else{
										$tiendas = $db->rawQuery("select * from pak_tiendas where idioma = $idioma and tipo NOT IN(196,199,193) order by nombre limit 12");
									}
...
<?php 
	...
	if($_GET['busqueda']){
		...
		$db->where('tipo', Array(196, 199,193), 'NOT IN');
